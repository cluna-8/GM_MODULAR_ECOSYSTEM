...
# Configuration
MEDICAL_API_URL = "http://healthcare-chat-api:7005"  # Internal Docker network
security = HTTPBearer()
...
async def list_tokens(
    ...
    """Change for join User to get username"""
...
async def list_sessions(
    ...
    sessions = db.query(DBSession).join(Token).join(User, Token.user_id == User.id).all()
    
    result = []
    for session in sessions:
        # Parse JSON fields BEFORE creating SessionResponse
        tools_used = []
        prompt_modes_used = []
        
        try:
            tools_used = json.loads(session.tools_used) if session.tools_used else []
        except:
            tools_used = []
            
        try:
            prompt_modes_used = json.loads(session.prompt_modes_used) if session.prompt_modes_used else []
        except:
            prompt_modes_used = []
        
        # Create SessionResponse with parsed data
        session_data = SessionResponse(
            ...
            prompt_modes_used=prompt_modes_used,
