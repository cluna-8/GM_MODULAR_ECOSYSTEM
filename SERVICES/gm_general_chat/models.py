...
class ChatRequest(BaseModel):
    """Modelo unificado para el endpoint /chat (con soporte para legado)"""
    ...
    prompt_mode: Optional[PromptMode] = Field(
        PromptMode.MEDICAL, description="Modo de prompt", validation_alias="IAType"
    )
    language: Optional[str] = Field(
        "auto", description="Idioma preferido (auto, es, en)"
    )
...
class ToolRequest(BaseModel):
    ...
    session: Optional[str] = Field(
        None, description="ID de sesión (opcional para tools directos)"
    )
    token: Optional[str] = Field(None, description="Token de usuario")
    max_results: Optional[int] = Field(
        3, description="Máximo número de resultados", ge=1, le=10
    )
    format_response: Optional[bool] = Field(
        True, description="Formatear respuesta con LLM"
    )
...
class PromptUpdateRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(
        0.1, description="Temperatura del LLM", ge=0.0, le=1.0
    )
    max_tokens: Optional[int] = Field(
        1000, description="Máximo tokens", ge=100, le=4000
    )
...
class FeedbackRequest(BaseModel):
    """Modelo para recibir feedback de expertos médicos (RLHF)"""

    session_id: str = Field(..., description="ID de sesión de la interacción")
    rating: int = Field(..., description="Calificación (1-5 o -1/1)", ge=-1, le=5)
    comment: Optional[str] = Field(None, description="Comentario del experto")
    expert_id: Optional[str] = Field(None, description="ID del médico que valida")
    is_dangerous: bool = Field(False, description="¿La respuesta era peligrosa?")
    suggested_response: Optional[str] = Field(
        None, description="Respuesta correcta sugerida"
    )
...
class StandardResponse(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp de respuesta"
    )
    ...
    usage: Optional[Dict[str, int]] = Field(
        None, description="Consumo detallado de tokens"
    )
...
class ChatResponse(StandardResponse):
    ...
    conversation_count: Optional[int] = Field(
        None, description="Número de mensajes en conversación"
    )
    prompt_mode_used: Optional[PromptMode] = Field(
        None, description="Modo de prompt usado"
    )
...
class ToolResponse(StandardResponse):
    ...
    search_term: Optional[str] = Field(
        None, description="Término de búsqueda procesado"
    )
    results_count: Optional[int] = Field(
        None, description="Número de resultados encontrados"
    )
...
def create_success_response(
    data: Dict[str, Any], message: str = "Success", session_id: str = None, **kwargs
) -> StandardResponse:
...
def create_error_response(
    message: str, error_code: str = None, session_id: str = None, **kwargs
) -> ErrorResponse:
    """Helper para crear respuestas de error"""
    return ErrorResponse(
        message=message, error_code=error_code, session_id=session_id, **kwargs
    )
